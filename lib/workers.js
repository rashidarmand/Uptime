...
// Lookup all the checks, get their data, send that data to a validator
workers.gatherAllChecks = ()=>{
  ...
  _data.list('checks', (err, checks)=>{
    if(!err && checks && checks.length > 0){
      ...
      console.log('Errror: Could not find any checks to process');
    }
  });
};
...
workers.validateCheckData = (originalCheckData)=>{
  ...
  if(originalCheckData.id &&
    ...
    originalCheckData.timeoutSeconds){
      ...
      console.log('Error: One of the checks in not properyly formatted. Skipping it');
    }
...
workers.init = ()=>{
  ...
  // Call a loop so the checks will execute later on their own
  workers.loop();
}
